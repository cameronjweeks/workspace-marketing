...
<header class="sticky top-0 z-50 border-b border-ink-700/60 bg-ink-950/85 backdrop-blur-md">
	...
	{#if mobileOpen}
		<nav class="border-t border-ink-700/60 px-4 py-4 lg:hidden">
			...
			{/each}
			<div class="mt-4">
				<a href={ctaPrimary.href} target="_blank" rel="noopener noreferrer" class="block rounded-lg bg-ws-500 px-4 py-2.5 text-center text-sm font-semibold text-white">{ctaPrimary.label}</a>
			</div>
		</nav>
	{/if}
</header>
